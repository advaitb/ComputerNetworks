        ...
        while (1){
            ...
            if (bytes_rcvd > 0)
            {
                ...
                if ((char)rcvID == (char)sentID)
                {
                    ...
                    clock_t end = clock();
                    printf("clock end %f\n", (double)end);
                    double time_taken = ((double)(end-begin) / CLOCKS_PER_SEC) * 1000 + offset; // Add the accumulated TO value
                    begin = clock();
            ...
            {
                offset = offset + PRRT; // if timeout, TO value should be added to estimate the RTT
